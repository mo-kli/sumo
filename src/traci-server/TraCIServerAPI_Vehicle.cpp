...
TraCIServerAPI_Vehicle::processSet(TraCIServer& server, tcpip::Storage& inputStorage,
    ...
    if (variable != libsumo::CMD_STOP && variable != libsumo::CMD_CHANGELANE
            ...
            && variable != libsumo::VAR_LC_DURATION
    ...
    try {
        switch (variable) {
            ...
            break;
            case libsumo::VAR_LC_DURATION: {
                int lcduration = 0;
                if (!server.readTypeCheckingInt(inputStorage, lcduration)) {
                    return server.writeErrorStatusCmd(libsumo::CMD_SET_VEHICLE_VARIABLE, "Setting lane change duration requires an integer.", outputStorage);
                }
